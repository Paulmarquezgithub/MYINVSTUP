document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateRevenueChart() {
    let investments = JSON.parse(localStorage.getItem("investments")) || [];
    const labels = investments.map(inv => inv.startupName);
    const revenueData = investments.map(inv => inv.potentialRevenue);
  ...
  function updateTimelineChart() {
    let investments = JSON.parse(localStorage.getItem("investments")) || [];
    const labels = investments.map(inv => inv.startupName);
    // Compute timeline progress for each investment.
    const progressData = investments.map(inv => {
      const investDate = new Date(inv.date);
      const estimatedEnd = new Date(inv.estimatedEnd);
      const totalDurationDays = (estimatedEnd - investDate) / (1000 * 60 * 60 * 24);
      let elapsedDays = (simulatedNow - investDate) / (1000 * 60 * 60 * 24);
      if (elapsedDays < 0) elapsedDays = 0;
      const progress = totalDurationDays > 0 ? Math.min(elapsedDays / totalDurationDays, 1) * 100 : 0;
    ...
    if (timelineChart) {
      ...
      timelineChart.data.datasets[0].data = progressData;
